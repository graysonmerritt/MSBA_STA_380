...
```
# Problem 3
In order to agree with the stats guru's conclusion, we first have to do our own analysis of the data
and see what findings we can come up with. 
...
Rent is LOWER for renovated buildings. This is not what we expected. Most green buildings however 
...
# Look at which buildings are "more desirable"
xtabs(~ class_a + green_rating, data=greenbuildings) %>%
  prop.table(margin=2)

# Look at difference in age
